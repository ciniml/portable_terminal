...
void show()    { g_visible.store(true, std::memory_order_release); }
...
void render() {
    ...
    // Layout: title band on top, two QR panels side by side, credential
    // lines under each QR, dismiss hint at the bottom. White background
    // for QR contrast (scanners want a light quiet zone).
    constexpr int kQrW    = 300;
    const int col_l = W / 4;          // centre of the left column
    const int col_r = (W * 3) / 4;    // centre of the right column
    const int qr_y  = 170;
